...
{
    public class PlayersListBase : ListBase<Player>
    {
        public async Task GetIndexDataEvent(string indexName)
        {
            ...
            if (indexName == nameof(Player))
                indexData = await GetIndexData<Player>(); 
            if (indexName == nameof(PersonType))
                indexData = await GetIndexData<PersonType>();
